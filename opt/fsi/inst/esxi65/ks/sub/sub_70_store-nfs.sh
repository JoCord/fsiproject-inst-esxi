#!/bin/sh
#
#   sub_70_store-nfs.sh - create nfs storage
#
...
if [ "$nfscfg" == "none" ]; then
   export nfscfg="$vipath/nfsstore.cfg"
   echo export nfscfg=$nfscfg >>$viconf
fi
            ...
            if [ "$nfsexport" != "none" ]; then
               ...
               if [ -d $nfspath ]; then
                  ...
                  if [ $retc -ne 0 ]; then
                     ...
                     infmsg "$ls   write nfs config to local repo"
                     repoentry="nfs: $storename : $nfsserver : $nfsexport"
                     tracemsg "$ls   nfs repo entry: $repoentry"
                     tracemsg "$ls   nfs repo : $nfscfg"
                     echo  "$repoentry" >>$nfscfg
                  fi
               fi
            else
